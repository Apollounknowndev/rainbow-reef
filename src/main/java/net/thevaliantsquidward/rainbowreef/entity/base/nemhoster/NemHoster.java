...
public abstract class NemHoster<V extends ReefVariant> extends ReefSchoolingFishEntity<V> {
    ...
    public NemHoster(EntityType<? extends ReefSchoolingFishEntity> type, Level level, int searchCD, int homeRadius, int restInterval, int restTime) {
        super(type, level, 200);
    ...
    public List<BlockPos> findNems() {
        ...
        Stream<PoiRecord> stream = poimanager.getInRange((holder) -> holder.is(ReefPOITypes.ANEMONES), blockpos, 100, PoiManager.Occupancy.ANY);

        return stream.map(PoiRecord::getPos).sorted(Comparator.comparingDouble((pos) -> pos.distSqr(blockpos))).collect(Collectors.toList());
    }
}
